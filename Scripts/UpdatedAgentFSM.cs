...
public class UpdatedAgentFSM : FSM
{
    ...
    {
        //Set the target position as the player position opposite diriction
        Quaternion targetRotation = Quaternion.LookRotation(destPos + transform.position); // was -
        transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, Time.deltaTime * curRotSpeed);

        //Go Forward
        transform.Translate(Vector3.forward * Time.deltaTime * curSpeed);
        //change to recover if away
        float dist = Vector3.Distance(transform.position, playerTransform.position);
        if (dist >= 30.0f)
        {
            print("Switched to Recover Position");
            curState = FSMState.Recover;
        }

        //return to Attacking if cannot escape and health very low
        else if (dist <= 20.0f && health <= 30)
        {
    ...
    protected void UpdateRecoverState()
    {
        ...
        //return to patrol state if player too far and health has recovered
        else if (dist >= 30.0f && health >= 100)
        {
            curState = FSMState.Patrol;
            RT = 0;
        }
    ...
    private void ShootBullet()
    {
        if (elapsedTime >= shootRate)
        {
            ...
            GameObject newBullet = Instantiate(Bullet,
                this.transform.position + new Vector3(1.5f, 1.0f, 0f),
